...
class menu:
    def __init__(self, managerNum):
        ...
        self.mgr_new = tkinter.Button(self.menu, text='账号管理', command=self.mgr_new)
...
class mgrQuery:
    ...
    def deleteAccount(self):
        """删除账号逻辑"""
        account_id = self.idEntry.get()
        account_type = "教师" if self.var.get() == 1 else "学生"

        if not account_id:
            self.promptLabel.configure(text='教师年龄不用输入，请输入要删除的账号ID')
            return

        # 确认对话框
        confirm = messagebox.askyesno("确认删除",
                                      f"确定要删除{account_type}账号：{account_id} 吗？")
        if not confirm:
            return

        if self.var.get() == 1:
            result = self.delete_teacher_account(account_id)
        else:
            result = self.delete_student_account(account_id)

        if result == 0:
            self.promptLabel.configure(text=f'该{account_type}账号已删除成功')
            self.clear_inputs()
        else:
            self.promptLabel.configure(text=f'{account_type}账号删除失败')

    def clear_inputs(self):
        """清空所有输入框"""
        for entry in [self.nameEntry, self.idEntry, self.sexEntry,
                      self.pswdEntry, self.ageEntry]:
            entry.delete(0, tkinter.END)

    def delete_teacher_account(self, account_id):
        """删除教师账号"""
        db = connect()
        cursor = db.cursor()
        sql = f"DELETE FROM tb_teacher WHERE teacherID='{account_id}'"
        try:
            cursor.execute(sql)
            affected_rows = cursor.rowcount
            db.commit()
            db.close()
            return 0 if affected_rows > 0 else 1
    ...
    def delete_student_account(self, account_id):
        ...
        db = connect()
        cursor = db.cursor()
        sql = f"DELETE FROM tb_student WHERE studentID='{account_id}'"
        try:
            cursor.execute(sql)
            affected_rows = cursor.rowcount
            db.commit()
...
class mgrNew:
    def __init__(self, frame, managerNum):
        ...
        self.pswdEntry = tkinter.Entry(self.frame,show='*')
        ...
        self.enterButton = tkinter.Button(self.frame, text='创建', command=self.createAccount)
        self.delButton = tkinter.Button(self.frame, text='删除', command=self.deleteAccount)
        self.updateButton = tkinter.Button(self.frame, text='更新', command=self.updateAccount)

        self.enterButton.place(x=INFO_TEXT_INDENT + 150,  # 基础偏移量
                               y=INFO_TEXT_LINE_SPACE * 4,
                               width=80, height=25)
        self.delButton.place(x=INFO_TEXT_INDENT + 240,  # 前一个按钮x坐标 + 宽度 + 间距
                             y=INFO_TEXT_LINE_SPACE * 4,
                             width=80, height=25)
        self.updateButton.place(x=INFO_TEXT_INDENT + 330,  # 前一个按钮x坐标 + 宽度 + 间距
                             y=INFO_TEXT_LINE_SPACE * 4,
                             width=80, height=25)
        ...
        if self.var.get() == 1:
            if len(name)!=0 and len(id)!=0 and len(sex)!=0 and len(pswd)!=0:
                if self.insert_teacher_account(name, id, sex, pswd) == 0:
                    self.promptLabel.configure(text='该教师账号已创建成功')
                else:
            ...
            if len(name) != 0 and len(id) != 0 and len(sex) != 0 and len(pswd) != 0 and len(age) != 0:
                if self.insert_student_account(name, id, sex, pswd, age) == 0:
                    self.promptLabel.configure(text='该学生账号已创建成功')
                else:
    ...
    def insert_teacher_account(self, name, id, sex, pswd):
        ...
        sql = f'INSERT INTO tb_teacher(teacherID, teacherName, teacherSex, teacherPswd)' \
              f'VALUES(\'{id}\', \'{name}\', \'{sex}\', SHA2(\'{pswd}\',256)) '
        try:
            cursor.execute(sql)
        except:
            return 1
    ...
    def insert_student_account(self, name, id, sex, pswd, age):
        ...
        sql = f'INSERT INTO tb_student(studentID, studentName, studentSex, studentPswd, studentAge)' \
              f'VALUES(\'{id}\', \'{name}\', \'{sex}\', SHA2(\'{pswd}\',256), {age}) '
        try:
            cursor.execute(sql)
        except:
            return 1
    ...
    def deleteAccount(self):
        ...
        if not account_id:
            self.promptLabel.configure(text='请输入要删除的账号ID')
            return
    ...
    def updateAccount(self):
        ...
        new_pswd = self.pswdEntry.get()
        new_age = self.ageEntry.get()

        if self.var.get() == 1:  # 教师
            if len(target_id) != 0 and len(new_name) != 0 and len(new_sex) != 0 and len(new_pswd) != 0:
                result = self.update_teacher(target_id, new_name, new_sex, new_pswd)
                if result == 0:
                    self.promptLabel.configure(text='该教师信息已成功更新')
                else:
                    self.promptLabel.configure(text='教师信息更新失败')
            else:
                self.promptLabel.configure(text='请填写所有必填字段')
        else:  # 学生
            if len(target_id) != 0 and len(new_name) != 0 and len(new_sex) != 0 and len(new_pswd) != 0 and len(
                    new_age) != 0:
                result = self.update_student(target_id, new_name, new_sex, new_pswd, new_age)
                if result == 0:
                    self.promptLabel.configure(text='该学生信息已成功更新')
                else:
                    self.promptLabel.configure(text='学生信息更新失败')
            else:
                self.promptLabel.configure(text='请填写所有必填字段')

    def update_teacher(self, tid, name, sex, pswd):
        db = connect()
        cursor = db.cursor()
        sql = f'''UPDATE tb_teacher SET 
                teacherName='{name}',
                teacherSex='{sex}',
                teacherPswd='{pswd}',
                WHERE teacherID='{tid}' '''
    ...
    def update_student(self, sid, name, sex, pswd, age):
        db = connect()
        cursor = db.cursor()
        sql = f'''UPDATE tb_student SET 
                studentName='{name}',
                studentSex='{sex}',
                studentPswd='{pswd}',
                studentAge={age}
