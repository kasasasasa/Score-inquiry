...
class menu:
    def __init__(self, StudentNum):
        ...
        self.stu_choose = tkinter.Button(self.menu, text='加入班级', command=self.stu_choose)
        self.stu_drop = tkinter.Button(self.menu, text='班级列表', command=self.stu_drop)
...
class stuInfo:
    def __init__(self, frame, studentNum):
        ...
        self.pswd_Entry = tkinter.Entry(self.frame, textvariable=self.pswdVar,show='*')
        # 修改密码
    ...
    def initialize(self):
        ...
        # 放置修改密码按钮，和密码提示
        self.frame.update()
        self.promptLabel.place(x=self.pswd_Entry.winfo_x(),
                               y=self.pswd_Entry.winfo_y() + self.pswd_Entry.winfo_height() + 5)
        self.commitButton.place(x=self.pswd_Entry.winfo_x() + self.pswd_Entry.winfo_width() + 5,
                                y=self.pswd_Entry.winfo_y() - 5)
    ...
    def set_student_pswd(self, pswd):
        db = connect()
        cursor = db.cursor()
        sql = f'UPDATE tb_student SET studentPswd= SHA2(\'{pswd}\',256) WHERE studentID = \'{self.studentNum}\''
        cursor.execute(sql)
        try:
            db.commit()
...
class stuChoose:
    def __init__(self, frame, studentNum):
        ...
        # 加入班级按钮
        self.chooseButton = tkinter.Button(self.frame, text='加入班级', command=self.chooseCourse)
        # 提示信息
        self.promptLabel = tkinter.Label(self.frame, text='一次加入一个班级，已选的班级不出现在列表里')
        # 选中的课程
        self.selected = None
    ...
    def chooseCourse(self):
        if self.selected is None:
            ...
            if self.insert_student_course(self.studentNum, self.selected['values'][0], self.selected['values'][2]) == 0:
                self.promptLabel.configure(text='加入班级成功！')
            else:
                self.promptLabel.configure(text='加入班级失败！')
        # 加入班级完成后更新班级列表
        self.displayInfo()
...
class stuDrop:
    def __init__(self, frame, studentNum):
        ...
        self.resultTable = ttk.Treeview(self.frame,
                                        ...
                                        yscrollcommand=self.resultScrollbar.set)
        # 退出班级按钮
        self.dropButton = tkinter.Button(self.frame, text='退出班级', command=self.dropCourse)
        # 提示信息
        self.promptLabel = tkinter.Label(self.frame, text='一次退一个班级，已退的班级不出现在列表里')
        # 选中的课程
        self.selected = None
    ...
    def dropCourse(self):
        if self.selected is None:
            ...
            if self.drop_student_course(self.studentNum, self.selected['values'][0], self.selected['values'][2]) == 0:
                self.promptLabel.configure(text='退出班级成功！')
            else:
                self.promptLabel.configure(text='退出班级失败！')
        # 加入班级完成后更新班级列表
        self.displayInfo()
